...
import { type PlatformRole, type TenantRole } from '@/types/core';
...
const MenuItem: React.FC<MenuItemProps> = ({ to, icon, children, isActive }) => (
  <Link
    to={to}
...
const Sidebar: React.FC = () => {
  ...
  const platformRole = user?.platform_role?.toUpperCase() as PlatformRole | undefined;
  const tenantRole = user?.tenant_role?.toLowerCase() as TenantRole | undefined;

  const isPlatformAdmin = platformRole === 'ADMIN' || platformRole === 'SUPER_ADMIN';
  const isHRAdmin = ['hr', 'admin'].includes(tenantRole as string);
  const isStaff = tenantRole === 'staff';

  const renderIcon = (path: string): React.ReactNode => {
